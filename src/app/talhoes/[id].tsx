...
import { ALERTAS_TALHAO, CULTURAS, STATUS_CFG, TIPOS_SOLO } from "@/data/talhaoConstants"
...
export default function EditarTalhao() {
  ...
  useEffect(() => {
    talhaoService.buscar(talhaoId).then((t) => {
      if (t) {
        setTalhao(t)
        setNome(t.nome)
  ...
  async function salvar() {
    ...
    setLoading(true)
    try {
      await talhaoService.atualizar(talhaoId, {
        nome: nome.trim(),
  ...
  async function executarExclusao() {
    setDeletando(true)
    try {
      await talhaoService.deletar(talhaoId)
      router.back()
    } catch {
